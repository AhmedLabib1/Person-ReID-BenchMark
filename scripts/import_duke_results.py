...
TEAMMATE_REF = "origin/feature/DukeMTMC-Benchmark"
...
def convert_metrics(
    payload: dict, model_key: str, dataset: str, efficiency_row: dict
) -> dict:
    ...
    if dataset == "msmt17":
        notes = (
            f"{notes} MSMT eval is MSMT17_V2 "
            f"({int(payload['query_samples'])} query / "
            f"{int(payload['gallery_samples'])} gallery; "
            "one unreadable image skipped in each split)."
        )
    metrics = {
    ...
    return {
        ...
        "efficiency": {
            "source": TEAMMATE_REF,
            "note": (
                "Teammate profiler on Tesla T4 at batch 16, wall-clock. "
...
def import_duke(efficiency: dict[str, dict[str, str]]) -> None:
    for model_id in DUKE_MODELS:
        our_key = f"duke_{model_id}"
        for dataset, protocol in (
            ("market1501", "single_image_l2"),
            ("mars", "uniform8_mean_raw_l2"),
            ("msmt17", "single_image_l2"),
        ):
            rel = f"benchmarks/results/{model_id}/{dataset}/{protocol}/metrics.json"
...
def main() -> None:
    ...
    import_duke(efficiency)
    print(f"Imported DukeMTMC teammate results into {OUT_DIR}")
