...
class CachedEmbeddings:
    ...
    def validate(self) -> None:
        ...
        if len(self.person_ids) != num_samples:
            raise ValueError(
                "person_ids length mismatch."
            )

        if len(self.camera_ids) != num_samples:
            raise ValueError(
                "camera_ids length mismatch."
            )

        if len(self.sample_ids) != num_samples:
            raise ValueError(
                "sample_ids length mismatch."
            )
...
def extract_tracklet_cache(
    *,
    ...
    if frame_batch_size <= 0:
        raise ValueError(
            "frame_batch_size must be > 0."
        )

    if tracklet_chunk_size <= 0:
        raise ValueError(
            "tracklet_chunk_size must be > 0."
        )
